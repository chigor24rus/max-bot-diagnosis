...
def handle_checklist_answer(sender_id: str, session: dict, payload: str):
    ...
    skip_logic = {
        ...
        34: 35,  # Уровень антифриза красный (na) → пропускаем к зеленому
        38: 39,  # Уровень антифриза зеленый (na) → пропускаем к синему
        42: 43,  # Уровень антифриза синий (na) → пропускаем к ГУР
        43: 45,  # Уровень жидкости ГУР (na) → пропускаем 44 (Состояние жидкости ГУР)
